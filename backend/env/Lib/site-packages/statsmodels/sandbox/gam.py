...
class AdditiveModel:
    ...
    def df_resid(self):
        ...
        '''
        return self.results.Y.shape[0] - np.array(
            [self.smoothers[i].df_fit() for i in range(self.exog.shape[1])]).sum()
...
class Model(GLM, AdditiveModel):
    ...
    def next(self):
        ...
        Z = _results.predict(self.exog) + \
            self.family.link.deriv(_results.mu) * (
                        Y - _results.mu)  # - _results.alpha #?added alpha
