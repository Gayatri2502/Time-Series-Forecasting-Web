...
format_def(char *buf, Py_ssize_t size, FortranDataDef def) {
    ...
    n = PyOS_snprintf(p, size, "array(%"
    NPY_INTP_FMT, def.dims.d[0]);
    if (n < 0 || n >= size) {
        return -1;
    }
    p += n;
    ...
    for (i = 1; i < def.rank; i++) {
        n = PyOS_snprintf(p, size, ",%"
        NPY_INTP_FMT, def.dims.d[i]);
        if (n < 0 || n >= size) {
            return -1;
        }
        p += n;
...
static void
set_data(char *d, npy_intp *f) {           /* callback from Fortran */
...
ndarray_from_pyobj(const int type_num,
    ...
    if ((intent & F2PY_INTENT_HIDE)
        ...
        if (PyArray_ITEMSIZE(arr) != elsize) {
            strcpy(mess, "failed to create intent(cache|hide)|optional array");
            sprintf(mess + strlen(mess), " -- expected elsize=%d got %"
            NPY_INTP_FMT, elsize, (npy_intp) PyArray_ITEMSIZE(arr));
            PyErr_SetString(PyExc_ValueError, mess);
            Py_DECREF(arr);
            return NULL;
        }
        ...
        */
        if ((!(intent & F2PY_INTENT_COPY)) &&
            ...
            if ((intent & F2PY_INTENT_INOUT || intent & F2PY_INTENT_INPLACE)
                ...
                : ((intent & F2PY_INTENT_C) ? PyArray_ISCARRAY_RO(arr) : PyArray_ISFARRAY_RO(
                            arr))) {
                if ((intent & F2PY_INTENT_OUT)) {
                    Py_INCREF(arr);
                }
...
check_and_fix_dimensions(const PyArrayObject *arr, const int rank,
    ...
    if (rank > PyArray_NDIM(arr)) { /* [1,2] -> [[1],[2]]; 1 -> [[1]]  */
        ...
        if (new_size != arr_size) {
            PyErr_Format(PyExc_ValueError,
                         "unexpected array size: new_size=%"
            NPY_INTP_FMT
            ", got array with arr_size=%"
            NPY_INTP_FMT
...
f2py_describe(PyObject *obj, char *buf) {
    ...
    */
    ...
    if (PyBytes_Check(obj)) {
        ...
        sprintf(localbuf, "%c%"
        NPY_INTP_FMT
        "-%s-scalar", PyArray_DESCR(arr)->kind, PyArray_ITEMSIZE(arr), Py_TYPE(obj)->tp_name);
        ...
        for (i = 0; i < PyArray_NDIM(arr); i++) {
            ...
            sprintf(localbuf + strlen(localbuf), "%"
            NPY_INTP_FMT
            ",", PyArray_DIM(arr, i));
        }
        sprintf(localbuf + strlen(localbuf), ")-%c%"
        NPY_INTP_FMT
        "-%s", PyArray_DESCR(arr)->kind, PyArray_ITEMSIZE(arr), Py_TYPE(obj)->tp_name);
    } else if (PySequence_Check(obj)) {
...
f2py_size_impl(PyArrayObject *var, ...) {
    ...
    if (dim == -1) {
        ...
        if (dim >= 1 && dim <= rank)
            ...
            fprintf(stderr, "f2py_size: 2nd argument value=%"
        NPY_INTP_FMT
        " fails to satisfy 1<=value<=%"
        NPY_INTP_FMT
        ". Result will be 0.\n", dim, rank);
    }
    va_end(argp);
