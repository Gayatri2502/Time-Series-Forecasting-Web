...
def train_sarima():
    try:
        ...
        forecast_steps = 12
        forecast_index = pd.date_range(start=ml_model.df.index[-1], periods=forecast_steps + 1,
                                       freq='M')[1:]
        forecast_series = pd.Series(
            results.get_forecast(steps=forecast_steps).predicted_mean.values, index=forecast_index)
